package gov.hygs.htgl.mj.entity;

import java.util.Date;


public class DdxxMx {
	private Integer mxid;
	private Integer ddbid;
	private String wlh;
	private String wlmc;
	private String cz;
	private String clgg;
	private String gg;
	private String jhrq;
	private Integer ddsl;
	private String hyjq;
	private Double 	dj; 

	private Double 	je; 
	private String bz;
	private String dw;
	private Integer sl;
	private Integer chmxid;
	private Integer chdid;

	private Double 	mjf; 
	private Double 	sbf; 
	private Date 	fkdjsj; 
	private Date 	kmdbsj; 
	private Date 	yqjysj; 
	private Date 	yjmjrq; 

	public Double getJe() {
		return je;
	}
	public void setJe(Double je) {
		this.je = je;
	}
	public Integer getMxid() {
		return mxid;
	}
	public String getGg() {
		return gg;
	}
	public void setGg(String gg) {
		this.gg = gg;
	}
	public void setMxid(Integer mxid) {
		this.mxid = mxid;
	}
	public Integer getDdbid() {
		return ddbid;
	}
	public void setDdbid(Integer ddbid) {
		this.ddbid = ddbid;
	}
	public String getWlh() {
		return wlh;
	}
	public void setWlh(String wlh) {
		this.wlh = wlh;
	}
	public String getWlmc() {
		return wlmc;
	}
	public void setWlmc(String wlmc) {
		this.wlmc = wlmc;
	}
	public String getCz() {
		return cz;
	}
	public void setCz(String cz) {
		this.cz = cz;
	}
	public String getClgg() {
		return clgg;
	}
	public void setClgg(String clgg) {
		this.clgg = clgg;
	}
	public String getJhrq() {
		return jhrq;
	}
	public void setJhrq(String jhrq) {
		this.jhrq = jhrq;
	}
	public Integer getDdsl() {
		return ddsl;
	}
	public void setDdsl(Integer ddsl) {
		this.ddsl = ddsl;
	}
	public String getHyjq() {
		return hyjq;
	}
	public void setHyjq(String hyjq) {
		this.hyjq = hyjq;
	}
	public Double getDj() {
		return dj;
	}
	public void setDj(Double dj) {
		this.dj = dj;
	}
	public String getBz() {
		return bz;
	}
	public void setBz(String bz) {
		this.bz = bz;
	}
	public String getDw() {
		return dw;
	}
	public void setDw(String dw) {
		this.dw = dw;
	}
	public Integer getSl() {
		return sl;
	}
	public void setSl(Integer sl) {
		this.sl = sl;
	}
	public Integer getChmxid() {
		return chmxid;
	}
	public void setChmxid(Integer chmxid) {
		this.chmxid = chmxid;
	}
	public Integer getChdid() {
		return chdid;
	}
	public void setChdid(Integer chdid) {
		this.chdid = chdid;
	}
	public Double getMjf() {
		return mjf;
	}
	public void setMjf(Double mjf) {
		this.mjf = mjf;
	}
	public Double getSbf() {
		return sbf;
	}
	public void setSbf(Double sbf) {
		this.sbf = sbf;
	}
	public Date getFkdjsj() {
		return fkdjsj;
	}
	public void setFkdjsj(Date fkdjsj) {
		this.fkdjsj = fkdjsj;
	}
	public Date getKmdbsj() {
		return kmdbsj;
	}
	public void setKmdbsj(Date kmdbsj) {
		this.kmdbsj = kmdbsj;
	}
	public Date getYjmjrq() {
		return yjmjrq;
	}
	public void setYjmjrq(Date yjmjrq) {
		this.yjmjrq = yjmjrq;
	}
	public Date getYqjysj() {
		return yqjysj;
	}
	public void setYqjysj(Date yqjysj) {
		this.yqjysj = yqjysj;
	}
	
	
	
}
